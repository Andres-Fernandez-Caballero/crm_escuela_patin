...
public function actualizarEstado($nuevoEstado)
   ...
   public function congelarDisciplinas()
   {
       if ($this->estado_pago === 'pagada') {
           ...
           foreach ($disciplinas as $disciplina) {
               $disciplina->update(['estado' => 'congelada']); 
         }
       }
   }
   ...
   public function calcularPrecioConRecargo()
   {
       $totalSinRecargo = $this->calcularTotal(); // Usa el método actual para calcular el total sin recargo
   
       // Si la fecha de pago es después del día 15, aplica el recargo del 15%
       if ($this->updated_at->day > 15) {
           $recargo = $totalSinRecargo * 0.15;
           return $totalSinRecargo + $recargo;
       }
   
       return $totalSinRecargo;
   }
